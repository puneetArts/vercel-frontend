...
import illustration_club from "../assets/images/club-illustration.png";
    ...
    const fetchCollegeUsers = async () => {
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/users/college`,
          {
    ...
    const fetchCollegeFeed = async () => {
      try {
        const [postsRes] = await Promise.all([
          ...
          axios.get(
            `${import.meta.env.VITE_API_URL}/api/achievements/college`,
            {
              headers: { Authorization: `Bearer ${user.token}` },
            },
          ),
      ...
      >
        ...
        <div className="my-profile-section">
          <div className="edit-prof">
            ...
            <Link to={`/profile/${user._id}`}>
              <div className="btn-my-profile">
                <span>{user.name}</span>
                <small
                  style={{ fontSize: "0.8rem", opacity: 0.7, color: "white" }}
                >
                  {user.college?.name || "College not specified"}
                </small>
              </div>
                  ...
                  >
                    ...
                    <div>
                      {isAlreadyFriend(u._id) ? (
                        ...
                        <button disabled className="btn-peer-req">
                          Request Sent
                        </button>
                      ) : (
                        <button
          ...
          <div className="null-section">
            ...
            <Link to="/clubs" className="club-link">
              <img src={illustration_club} alt="hero" />
            </Link>
            <Link to="/clubs" className="club-link">
              Explore Latest Clubs
            </Link>
          </div>
        ...
        <div>
          <div
            className="college-feed"
            style={{ maxWidth: "100%", padding: "1rem", borderRadius: 8 }}
          >
            <h2 style={{ color: "white" }}>
              See what's up <span style={{ color: "#a4a0a0" }}>@</span>
              <small
                style={{ opacity: 0.7, color: "#F79B72", fontSize: "20px" }}
              >
                {user.college?.name || "College not specified"}
              </small>{" "}
            </h2>
